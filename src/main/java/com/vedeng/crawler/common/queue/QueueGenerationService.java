package com.vedeng.crawler.common.queue;
...
public class QueueGenerationService {
    ...
    /**
       根据业务与服务器性能自行配置 这里我配置的是最多1000个任务
    *  LinkedBlockingQueue构造的时候若没有指定大小，则默认大小为Integer.MAX_VALUE
    * @Author:strange
    * @Date:21:37 2020-03-07
    */
    private final LinkedBlockingQueue<QueueTaskHandler> tasks = new LinkedBlockingQueue<QueueTaskHandler>(2000);
    ...
    */
    ...
    public void activeService() {
        ...
        if (service.isShutdown()) {
            service = new ThreadPoolExecutor(4, 4,
                    0L, TimeUnit.MILLISECONDS,  new LinkedBlockingQueue<Runnable>(1024),
                    new ThreadFactoryBuilder().setNameFormat("QueueTaskHandler-pool-%d").build(), new ThreadPoolExecutor.AbortPolicy());
            init();
